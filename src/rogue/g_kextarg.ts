 ...
 *
 * The re-release asks the engine's nav mesh (`gi.GetPathToGoal`) for a walked
 * path length and, WHEN THE ENGINE REPORTS NO NAV DATA
 * (`PathReturnCode::NoNavAvailable`), falls back to the straight-line squared
 * distance. This module's GameImports has no nav-mesh entry point at all,
 * which is precisely the "no nav available" case -- so this is the
 * re-release's own documented fallback path, not an invented one.
 */
