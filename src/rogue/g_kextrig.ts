...
// ONLY in the 2023 re-release game DLL, translated into the Ground Zero
// (rogue) module.
//
// Source: our own sibling src/game/g_kextrig.ts, which translated
//   src/kexgame/g_trigger.ts        -> trigger_fog
//   src/kexgame/bots/bot_utils.ts   -> info_nav_lock
// (this repo's ports of quake2-rerelease-dll/rerelease/g_trigger.cpp and
// bots/bot_utils.cpp). Ported rather than imported -- see g_kexent.ts.
//
// Classnames landed in this file, with the shipped rogue maps that place them:
//   trigger_fog    -- rmine2, rlava2
//   info_nav_lock  -- rbase1
//
// TRANSLATION NOTES:
...
//     tail (see m_soldier.ts).
//   * kexgame's TouchFn is `(self, other, tr, otherTouchingSelf)`; this
//     module's touch signature is `(self, other, plane, surf)`. trigger_fog's
//     touch reads neither the trace nor the flag, so the translation is exact.
//   * kexgame's level.time is a millisecond GTime; this module's level.time is
//     a float in SECONDS, FRAMETIME (0.1) is one server frame.
...
//  * info_nav_lock -- the FL_KEX_LOCKED bit toggles; no Ground Zero door code
//                     tests it.
...
import { G_Find, G_FreeEdict, G_PickTarget } from "./g_utils";
...
/**
 * The re-release keeps a player's wanted fog on `client->pers.wanted_fog`,
 ...
 *
 * Ground Zero's ClientPersistantT declares none of those three, and
 * rogue/g_local.h is a faithful transcription of the 1998 header this port
 * does not want to grow with 2023 fields, so the same state lives here keyed
 * by the player's edict number (1..maxclients). Not serialized by g_save.ts's
 * ClientPersistantT walk, which is moot while nothing transmits it -- see
 * trigger_fog_touch's degradation note.
 */
 ...
 * height-fog values land in the per-client state exactly as the re-release
 * lands them in client->pers -- the wait debounce, the movetarget
 * value-storage indirection (rlava2's trigger_fog targets a `fog_values`
 * info_notnull that carries the whole heightfog_* block), the INSTANTANEOUS
 * zero-transition, the BLEND positional lerp across the trigger's movedir
 * axis, the FORCE/direction-of-travel on-vs-off decision, and both
 * AFFECT_FOG and AFFECT_HEIGHTFOG payloads.
 *
 * What is missing is transmission: the re-release publishes fog through a
 ...
 *   * The re-release rejects a target that is not `SVF_DOOR`. Ground Zero has
 *     only SVF_NOCLIENT / SVF_DEADMONSTER / SVF_MONSTER / SVF_DAMAGEABLE, so
 *     the "is this a door" test is made by classname against the two door
 *     entities instead. Same set of entities in practice.
 *   * `FL_LOCKED` becomes FL_KEX_LOCKED (g_kexent.ts, a free flag bit).
 *
 * NOTE ON func_door_secret2: rogue adds its own second secret-door entity,
 * but the re-release's own door check is SVF_DOOR, which rogue's
 * func_door_secret/func_door_secret2 do not carry either -- so the two-name
 * classname test above is the faithful equivalent, not a narrowing.
 *
 * DEGRADATION: no Ground Zero door code tests a locked flag -- func_door has
 * no lock concept at all -- so the toggle is inert. The entity spawns,
 * validates its targetname/target, and its use wiring works. rbase1's
 * info_nav_lock (targetname t98 -> target t10) therefore links up and
 * responds without changing door behavior.
 */
