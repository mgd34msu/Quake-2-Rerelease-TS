...
import { findradius, G_FreeEdict, G_Spawn, vectoangles, vectoangles2, vectoyaw } from "./g_utils";
...
import { infront, visible as visibleLocal } from "./g_ai";
...
// RERELEASE CONTENT PORT -- g_weapon.c's `check_dodge` is `static` there and
// src/game/g_weapon.ts is owned by another agent, so this file carries its
// own copy (identical, including the widened 4-argument monsterinfo.dodge).
function check_dodge(self: EdictT, start: Vec3, dir: Vec3, speed: number): void {
  ...
  if (tr.ent !== null) {
    ...
    if ((hit.svflags & SVF_MONSTER) !== 0 && hit.health > 0 && hit.monsterinfo.dodge !== null && infront(hit, self)) {
      const v = vec3();
...
// =========================================================================
// RERELEASE CONTENT PORT -- XATRIX PROJECTILES (xatrix/g_weapon.c)
//
// These three belong to g_weapon.c in the xatrix source. src/game/g_weapon.ts
// is owned by another agent (which may add the MONSTER-side xatrix helpers
// there), so this module's player-weapon copies live here instead. Flagged
// to the coordinator as a possible duplicate.
//
// NOTE -- name collision across packs: xatrix/g_weapon.c also defines a
// `fire_heat` (the heat-seeking rocket) that is a COMPLETELY DIFFERENT
// function from rogue/g_newweap.c's `fire_heat` (the plasma-beam hitscan)
// already exported above. Different parameters, different behaviour, same
// name. This file keeps rogue's; xatrix's must not be added here under that
// name. Reported to the coordinator.
// =========================================================================

function requireOwner(ent: EdictT): EdictT {
  if (ent.owner === null) {
    throw new Error("xatrix projectile: self.owner is null (C dereferences it unconditionally here)");
  }
  return ent.owner;
}
...
export function ionripper_touch(self: EdictT, other: EdictT, plane: CplaneT | null, surf: CsurfaceT | null): void {
  ...
  // fire_ionripper's own immediate self-trace, which passes NULL.
...
export function fire_ionripper(
  self: EdictT,
  start: Vec3,
  dir: Vec3,
  damage: number,
  ...
  // xatrix/g_weapon.c: the ion ripper blade ricochets flat off walls rather
  // than arcing under gravity, which is what MOVETYPE_WALLBOUNCE exists for
  // (stronger backoff, no gravity, re-orients to the new travel direction,
  // never settles on the ground). The enum member and g_phys.ts's four
  // WALLBOUNCE branches are both ported, so this is the real movetype now.
...
function Trap_Think(ent: EdictT): void {
  ...
  for (;;) {
    ...
    if (!visibleLocal(ent, target)) continue;
    if (best === null) {
      best = target;
      continue;
    }
...
// -------------------------------------------------------------------------
// Savegame function registry -- every think/touch/die/pain callback this
// file installs on an edict, so a save that lands mid-flight restores the
// real function (see g_save.ts's registerSaveFunction name registry).
// -------------------------------------------------------------------------

import { registerSaveFunction } from "./g_save";

registerSaveFunction("g_newweap:flechette_touch", flechette_touch);
registerSaveFunction("g_newweap:Prox_Explode", Prox_Explode);
registerSaveFunction("g_newweap:prox_die", prox_die);
registerSaveFunction("g_newweap:Prox_Field_Touch", Prox_Field_Touch);
registerSaveFunction("g_newweap:prox_seek", prox_seek);
registerSaveFunction("g_newweap:prox_open", prox_open);
registerSaveFunction("g_newweap:prox_land", prox_land);
registerSaveFunction("g_newweap:Nuke_Quake", Nuke_Quake);
registerSaveFunction("g_newweap:nuke_die", nuke_die);
registerSaveFunction("g_newweap:Nuke_Think", Nuke_Think);
registerSaveFunction("g_newweap:nuke_bounce", nuke_bounce);
registerSaveFunction("g_newweap:tesla_remove", tesla_remove);
registerSaveFunction("g_newweap:tesla_die", tesla_die);
registerSaveFunction("g_newweap:tesla_blow", tesla_blow);
registerSaveFunction("g_newweap:tesla_zap", tesla_zap);
registerSaveFunction("g_newweap:tesla_think_active", tesla_think_active);
registerSaveFunction("g_newweap:tesla_activate", tesla_activate);
registerSaveFunction("g_newweap:tesla_think", tesla_think);
registerSaveFunction("g_newweap:tesla_lava", tesla_lava);
registerSaveFunction("g_newweap:blaster2_touch", blaster2_touch);
registerSaveFunction("g_newweap:tracker_pain_daemon_think", tracker_pain_daemon_think);
registerSaveFunction("g_newweap:tracker_touch", tracker_touch);
registerSaveFunction("g_newweap:tracker_fly", tracker_fly);
registerSaveFunction("g_newweap:ionripper_sparks", ionripper_sparks);
registerSaveFunction("g_newweap:ionripper_touch", ionripper_touch);
registerSaveFunction("g_newweap:plasma_touch", plasma_touch);
registerSaveFunction("g_newweap:Trap_Think", Trap_Think);

// RERELEASE CONTENT PORT -- rogue/g_monster.c's monster_fire_heat, the
// muzzle-flash wrapper around this file's heatbeam fire_heat above.
// It lives here rather than in g_monster.ts because this module already
// has a monster_fire_heat: xatrix's own (xatrix/g_monster.c), the
// heat-seeking rocket the chick-heat fires through g_weapon.ts's
// fire_heat. The two are different weapons that happen to share a name
// in their respective packs; src/game keeps them apart the same way, by
// hosting one in g_weapon.ts and the other in g_monster.ts. Xatrix's
