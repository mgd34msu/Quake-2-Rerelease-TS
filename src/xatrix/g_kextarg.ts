...
// re-release game DLL, translated into The Reckoning (Xatrix) module.
//
// Classnames landed in this file:
//   target_poi
//
// target_poi is the single most widely placed re-release-only entity there is
// (115 shipped maps overall); every re-release Xatrix campaign map places it
// -- badlands, industry, outbase, ... -- and the frozen Xatrix DLL drops all
// of them with "target_poi doesn't have a spawn function".
//
// SOURCE: src/game/g_kextarg.ts's SP_target_poi and its two helpers, which are
// that module's translation of quake2-rerelease-dll/rerelease/g_target.cpp
// (via src/kexgame/g_target.ts). This module is a HARD FORK of src/game with
// its own EdictT and its own spawn table -- there is no cross-module import
...
//
// TRANSLATION NOTES (carried over from src/game/g_kextarg.ts):
...
//   * `deathmatchEnabled()` -> the classic `deathmatch->value` idiom, wrapped
//     as kexDeathmatch() in g_kexent.ts.
//   * `gi.Com_Print` -> `gi.dprintf`.
//
// ===========================================================================
// PROTOCOL-34 DEGRADATION DOCUMENTED IN THIS FILE
// ===========================================================================
//  * target_poi -- full server-side POI selection and stage bookkeeping runs;
//    there is no compass/objective HUD to present it. See SP_target_poi.
...
// ===========================================================================
// target_poi  (g_target.cpp:1593-1803 / src/game/g_kextarg.ts:608-800)
// ===========================================================================
 ...
 * The re-release compass/objective marker.
 *
 ...
 * level_locals_t. What is missing is the PRESENTATION: the rerelease draws the
 * POI as a compass arrow and objective marker from its own client-side HUD
 * layout, fed from those level fields through the extended player_state. The
 * Xatrix HUD (p_hud.ts's statusbar program) has no compass, no objective
 * marker and no slot to add one, and protocol 34's player_state carries no
 * field for it. So the POI is fully tracked and never shown.
 *
 * The image lookup still runs, so the "friend" / custom POI icon is precached
 * into CS_IMAGES exactly as the rerelease precaches it.
 */
