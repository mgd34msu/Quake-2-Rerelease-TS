...
import { below, inback, PredictAim, vectoyaw2 } from "./g_newai";
import {
  AI_CHARGING,
  AI_DO_NOT_COUNT,
  AI_HOLD_FRAME,
...
import { flyer_fireleft, flyer_fireright, flyer_run } from "./m_flyer";
import { monsterFlashOffset } from "./m_flash";
import { BossExplode } from "./m_supertank";
import * as F from "./m_carrier_frames";
import { FRAME_attak201, FRAME_attak217 } from "./m_flyer_frames";
...
// ROGUE -- rogue/m_flyer.c's circle-strafe move table (there named
// flyer_move_attack3), assigned directly onto a spawned monster_flyer
// escort's monsterinfo.currentmove below (CarrierSpawn), bypassing the
// flyer's own attack-choice function entirely. Built locally here (rather
// than in src/game/m_flyer.ts) so that m_flyer.ts's flyer_attack -- shared
// by every classic flyer -- does not gain rogue's ungated circle-strafe
// replacement; only flyer_run/flyer_fireleft/flyer_fireright (exported,
// unmodified) are reused. See src/game/m_flyer.ts's export notes on those
// three functions for the full rationale.
const flyer_frames_attack3: MframeT[] = [
  mf(ai_charge, 10),
  mf(ai_charge, 10),
  mf(ai_charge, 10),
  mf(ai_charge, 10, flyer_fireleft), // left gun
  mf(ai_charge, 10, flyer_fireright), // right gun
  mf(ai_charge, 10, flyer_fireleft), // left gun
  mf(ai_charge, 10, flyer_fireright), // right gun
  mf(ai_charge, 10, flyer_fireleft), // left gun
  mf(ai_charge, 10, flyer_fireright), // right gun
...
const flyer_move_attack3 = mkmove(FRAME_attak201, FRAME_attak217, flyer_frames_attack3, flyer_run);
// ROGUE
  ...
  if (FindSpawnPoint(startpoint, flyer_mins, flyer_maxs, spawnpoint, 32)) {
    ...
    if (self.enemy !== null && self.enemy.inuse && self.enemy.health > 0) {
      ...
      if (mytime === 1) {
        ...
        // NOT PORTED: rogue/m_carrier.c assigns `flyer_move_kamikaze` here
        // (the move table lives in rogue/m_flyer.c's own SP_monster_kamikaze
        // family, along with the kamikaze self-destruct think functions).
        // monster_kamikaze is not one of this round's 8 assigned classnames,
        // and CreateMonster(..., "monster_kamikaze") above already silently
        // degrades (ED_CallSpawn logs "doesn't have a spawn function" and
        // leaves `ent` as a blank, harmless edict with no model/solid/think)
        // since nothing registers that classname in g_spawn.ts's table yet.
        // Net effect: the carrier's 1-in-3 "kamikaze escort" variant spawns
        // an inert placeholder instead of a suicide flyer until another
        // porting round adds monster_kamikaze; the other two variants
        // (mytime 1 and 3, real monster_flyer escorts using the circle-
        // strafe move table below) are fully functional.
        ent.mass = 100;
...
registerSaveMmove("m_carrier:flyer_move_attack3", flyer_move_attack3);
