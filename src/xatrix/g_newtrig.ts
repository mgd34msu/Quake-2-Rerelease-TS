...
// october 1997
//
// Pack-only file (no baseq2 sibling): trigger_teleport (a rogue-specific
// teleporter trigger, distinct from vanilla's own misc_teleporter/
// misc_teleporter_dest in src/game/g_misc.ts -- ported here under its own
// classname per the port brief rather than merged into the vanilla ones)
// plus trigger_disguise, which marks touching entities with FL_DISGUISED so
// monsters won't recognize them (see g_newai.ts's has_valid_enemy/disguise
// checks -- owned by another unit).
...
// referenced only in the QUAKED comment/spawnflag bits above; kept (unused)
// to document the full spawnflag set the way the C header's #defines do.
void TELEPORT_PLAYER_ONLY;
void TELEPORT_SILENT;
void TELEPORT_CTF_ONLY;
...
// -------------------------------------------------------------------------
registerSaveFunction("g_newtrig:trigger_teleport_touch", trigger_teleport_touch);
registerSaveFunction("g_newtrig:trigger_teleport_use", trigger_teleport_use);
registerSaveFunction("g_newtrig:trigger_disguise_touch", trigger_disguise_touch);
registerSaveFunction("g_newtrig:trigger_disguise_use", trigger_disguise_use);
