// m_daedalus.ts -- rogue's hover/Daedalus, hosted alongside this module's
// own m_hover.ts.
//
// RERELEASE CONTENT PORT. This module already owns a faithful vanilla
// m_hover.ts (`monster_hover`), and rogue's m_hover.c is not an extension of
// it: rogue REPLACED several of the vanilla move tables (start_attack2 /
// attack2 / end_attack2 in, stop1 / takeoff / land / forward / backward out)
// and added hover_blocked. Merging the two would change `monster_hover`,
// which is xatrix content and stays exactly as it is.
//
// So rogue's version lives here as its own module, verbatim from
// src/game/m_hover.ts, and only the NEW classname `monster_daedalus` is
// wired to it in g_spawn.ts. `monster_hover` keeps calling this module's
// m_hover.ts. The spawn entry point is renamed SP_monster_daedalus so the
// two exports cannot be confused, and every savegame registry key is
// re-prefixed "m_daedalus:" so it cannot overwrite m_hover.ts's own
// registrations (the registry is a plain Map keyed by that string).
...
self.classname === "monster_daedalus" in SP_monster_daedalus), adds a
...
// rogue/m_hover.c applies this to BOTH monster_hover and monster_daedalus
// (they share this one hover_attack function) -- but that would change
// classic monster_hover's combat feel (always attack1 in vanilla 3.21) for
// every classic map, since monster_hover is one of baseq2's own classnames.
// RESTRICTED to monster_daedalus here; classic monster_hover keeps the
// original unconditional hover_move_attack1 below. See this file's porting
// report for the full rationale (same restriction pattern as g_ai.ts's
// ROGUE_MONSTER_CLASSNAMES gate).
...
function hover_pain(self: EdictT, _other: EdictT, _kick: number, damage: number): void {
  ...
  if (damage <= 25) {
    ...
    // between pain1 and pain2 instead of always pain1. RESTRICTED to
    // monster_daedalus: rogue/m_hover.c applies this to monster_hover too,
    // but that changes classic hover's high-damage pain animation (always
    // pain1 in vanilla 3.21) for every classic map. See hover_attack's
    // porting note above for the same restriction pattern.
...
*/
export function SP_monster_daedalus(self: EdictT): void {
  ...
  // its own sound set.
  if (self.classname === "monster_daedalus") {
    // monsterinfo.blocked wasn't set at all for classic monster_hover before
    // this port (rogue/m_hover.c sets it unconditionally for both
    // classnames); leaving it null for monster_hover keeps m_move.ts's new
    // SV_NewChaseDir blocked-fallback inert for that classic classname,
    // matching vanilla 3.21. See hover_attack's porting note above for the
...
registerSaveFunction("m_daedalus:hover_deadthink", hover_deadthink);
registerSaveFunction("m_daedalus:hover_pain", hover_pain);
registerSaveFunction("m_daedalus:hover_die", hover_die);
registerSaveFunction("m_daedalus:hover_stand", hover_stand);
registerSaveFunction("m_daedalus:hover_walk", hover_walk);
registerSaveFunction("m_daedalus:hover_run", hover_run);
registerSaveFunction("m_daedalus:hover_start_attack", hover_start_attack);
registerSaveFunction("m_daedalus:hover_sight", hover_sight);
registerSaveFunction("m_daedalus:hover_search", hover_search);
registerSaveFunction("m_daedalus:hover_blocked", hover_blocked);
registerSaveMmove("m_daedalus:hover_move_stand", hover_move_stand);
registerSaveMmove("m_daedalus:hover_move_pain3", hover_move_pain3);
registerSaveMmove("m_daedalus:hover_move_pain2", hover_move_pain2);
registerSaveMmove("m_daedalus:hover_move_pain1", hover_move_pain1);
registerSaveMmove("m_daedalus:hover_move_walk", hover_move_walk);
registerSaveMmove("m_daedalus:hover_move_run", hover_move_run);
registerSaveMmove("m_daedalus:hover_move_death1", hover_move_death1);
registerSaveMmove("m_daedalus:hover_move_start_attack", hover_move_start_attack);
registerSaveMmove("m_daedalus:hover_move_attack1", hover_move_attack1);
registerSaveMmove("m_daedalus:hover_move_end_attack", hover_move_end_attack);
registerSaveMmove("m_daedalus:hover_move_start_attack2", hover_move_start_attack2);
registerSaveMmove("m_daedalus:hover_move_attack2", hover_move_attack2);
registerSaveMmove("m_daedalus:hover_move_end_attack2", hover_move_end_attack2);
