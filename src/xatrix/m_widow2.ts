...
import { G_ProjectSource2, PickCoopTarget, PredictAim, realrange, vectoangles2, vectoyaw2 } from "./g_newai";
import { ThrowGib, ThrowHead } from "./g_misc";
import { M_ChangeYaw } from "./m_move2";
import { monster_fire_tracker, monster_think, walkmonster_start, CreateGroundMonster, FindSpawnPoint, SpawnGrow_Spawn } from "./g_monster";
// RERELEASE CONTENT PORT: rogue's monster_fire_heat (the heatbeam) is
// hosted in g_newweap.ts in this module -- g_monster.ts already exports
// xatrix's own, unrelated monster_fire_heat (the heat-seeking rocket).
import { monster_fire_heat } from "./g_newweap";
...
import { G_Find, G_FreeEdict, G_ProjectSource, G_Spawn, vectoangles } from "./g_utils";
import { fire_hit } from "./g_weapon";
import * as F from "./m_widow2_frames";

// ClipGibVelocity/gib_die/gib_touch are exported from rogue/g_misc.c, but
// src/game/g_misc.ts is outside this round's edit ownership and does not
// export its own (presumably very similar) private equivalents. Ported
// locally here -- pure, self-contained helpers with no dependency on
// anything g_misc.ts-specific -- since this file (widow2's own gib-throwing
// code) is their only consumer in this round's SCOPE.
