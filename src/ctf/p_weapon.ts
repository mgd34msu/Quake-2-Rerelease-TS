// p_weapon.c
//
// RERELEASE CONTENT PORT -- the mission-pack player weapons the two
// re-release CTF maps (q2kctf1.bsp, q2kctf2.bsp) hand the player, lifted
// from src/rogue/p_weapon.ts (rogue/p_weapon.c) and src/xatrix/p_weapon.ts
// (xatrix/p_weapon.c):
//   rogue:  P_DamageModifier / damage_multiplier (Quad + Double Damage
//           stack), P_ProjectSource2, Throw_Generic, Weapon_Prox,
//           Weapon_Tesla, Weapon_ProxLauncher, Weapon_ChainFist,
//           Weapon_Disintegrator, Weapon_ETF_Rifle, Weapon_Heatbeam.
//   xatrix: Weapon_Ionripper, Weapon_Phalanx, Weapon_Trap.
//
...
//     grenade release (a real rogue bug, preserved as-is in
//     src/rogue/p_weapon.ts) and moves the throw origin from
//     (8,8,viewheight-8)/P_ProjectSource to (2,6,viewheight-14)/
//     P_ProjectSource2. Applying that here would change how CTF's hand
//     grenade throws and how its kills are attributed, so `Weapon_Grenade`
//     keeps baseq2's state machine and its held=false release.
//     `Throw_Generic` is still ported, and drives the two NEW throwables
//     (Weapon_Prox / Weapon_Tesla) exactly as rogue does.
...
//     `NoAmmoWeaponChange`'s preference order. It is kept here (a CTF
...
//     (item_double, which q2kctf1/q2kctf2 both place) stacks with them the
//     way it does with every other weapon in this module.
//
// NOT PORTED from the siblings: xatrix's `is_quadfire`/`Use_Weapon2`
// (item_quadfire is not placed by any map in this module's game dir, so the
// double-shot flag would never be set and the HyperBlaster/Railgun swap
// would have nothing to swap to), and rogue's FL_DISGUISED branch in
// PlayerNoise (this module hosts no stalker/disguise AI).
//
...
// ROGUE -- `static byte damage_multiplier;`. Quad and rogue's Double Damage
// stack into one multiplier; every `if (is_quad) damage *= 4` below becomes
// `damage *= damage_multiplier`, which is still exactly 4 when only Quad is
// active, so classic play is unchanged.
...
// extra `up` basis vector). Mirrored here rather than added to g_utils.ts
// because it has exactly one consumer, the ported grenade-family and ETF
// rifle fire functions below.
...
export function NoAmmoWeaponChange(ent: EdictT): void {
  ...
  // file's header note 2 -- kept here so classic CTF play is unchanged).
...
function weapon_grenade_fire(ent: EdictT, held: boolean): void {
  ...
  if (weapon.tag === AmmoT.AMMO_GRENADES) {
    ...
    // baseq2/CTF's origin is kept so classic play is byte-identical.
...
// CTF's Weapon_Grenade above is left on baseq2's own hand-rolled state
// machine (see this file's header note 1); this drives the two NEW
// throwables, Weapon_Prox and Weapon_Tesla, exactly as rogue/p_weapon.c
// does.
//
// Faithful to rogue/p_weapon.c: `fire(ent, true)` is called with held=true
// at BOTH the "detonate in hand" branch AND the normal release
...
//
// NOTE this deliberately does NOT go through CTFApplyHaste the way this
// file's Weapon_Generic wrapper does: rogue's Throw_Generic has no such
// hook, and a haste-doubled throwable would advance the cook timer twice
// per frame. Prox/tesla therefore throw at their normal rate under Time
// Accel, which is also what CTF's own Weapon_Grenade does (Weapon_Grenade
// is likewise not wrapped).
//========
function Throw_Generic(
  ...
  if (client.weaponstate === WeaponstateT.WEAPON_READY) {
    ...
    for (const pf of pause_frames) {
      if (client.ps.gunframe === pf) {
        // rand()&15, see Weapon_Generic2's identical comment
        if (Math.floor(Math.random() * 16) !== 0) return;
      }
    }
...
//======================================================================
// RERELEASE CONTENT PORT -- ROGUE MODS BELOW (rogue/p_weapon.c), ported
// from src/rogue/p_weapon.ts. These are the weaponthink state machines for
// weapon_chainfist / weapon_disintegrator / weapon_etf_rifle /
// weapon_plasmabeam, which q2kctf2.bsp places.
//======================================================================
...
//======================================================================
// RERELEASE CONTENT PORT -- XATRIX MODS BELOW (xatrix/p_weapon.c), ported
// from src/xatrix/p_weapon.ts. These are the weaponthink state machines for
// weapon_boomer / weapon_phalanx (q2kctf1.bsp) and ammo_trap's thrown trap
// (q2kctf1.bsp).
//======================================================================
...
function weapon_ionripper_fire(ent: EdictT): void {
  ...
  // weapon in this module (see this file's header note 3).
  if (is_quad) {
    damage *= damage_multiplier;
    kick *= damage_multiplier;
  }
...
export function Weapon_Trap(ent: EdictT): void {
  ...
  if (client.weaponstate === WeaponstateT.WEAPON_READY) {
    ...
    if (
      ...
      // rand()&15, see the comment in Weapon_Generic2
      if (Math.floor(Math.random() * 16) !== 0) return;
    }
