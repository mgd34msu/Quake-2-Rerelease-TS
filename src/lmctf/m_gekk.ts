...
import { SolidT, SVF_DEADMONSTER, SVF_MONSTER } from "./game";
...
// xatrix/g_misc.c: `// RAFAEL` -- ThrowGibACID/ThrowHeadACID (acid-blooded
// gib effect, EF_GREENGIB) plus their VelocityForDamage/ClipGibVelocity/
// gib_die helpers. Ported here as module-private functions rather than
// added to g_misc.ts (owned by another agent, do-not-edit, and no other
// xatrix monster in this port's slice needs the acid-gib variant) --
// otherwise identical to g_misc.ts's ThrowGib/ThrowHead/VelocityForDamage/
// ClipGibVelocity/gib_die.
function gekkacid_VelocityForDamage(damage: number, v: Vec3): void {
  v[0] = 100.0 * crandom();
  v[1] = 100.0 * crandom();
...
function gekkacid_ClipGibVelocity(ent: EdictT): void {
...
function gekkacid_gib_die(self: EdictT, _inflictor: EdictT, _attacker: EdictT, _damage: number, _point: Vec3): void {
  G_FreeEdict(self);
}

// RAFAEL
function ThrowGibACID(self: EdictT, gibname: string, damage: number, gibType: number): void {
  const gib = G_Spawn();
  ...
  if (gibType === GIB_ORGANIC) {
    ...
    // is commented out in the C source, same as g_misc.ts's non-acid
    // ThrowGib.
    vscale = 3.0;
  } else {
  ...
  const vd = vec3();
  gekkacid_VelocityForDamage(damage, vd);
  VectorMA(self.velocity, vscale, vd, gib.velocity);
  gekkacid_ClipGibVelocity(gib);
...
// RAFAEL
function ThrowHeadACID(self: EdictT, gibname: string, damage: number, gibType: number): void {
  self.s.skinnum = 0;
  self.s.frame = 0;
  ...
  // s.renderfx, matching xatrix/g_misc.c's ThrowHeadACID exactly.
  self.s.effects |= RF_FULLBRIGHT;
  self.s.sound = 0;
  self.flags |= FL_NO_KNOCKBACK;
  self.svflags &= ~SVF_MONSTER;
  self.takedamage = DamageT.DAMAGE_YES;
  self.die = gekkacid_gib_die;
  self.dmg = 2;
