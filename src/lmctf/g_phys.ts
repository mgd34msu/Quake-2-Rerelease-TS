// Ports lmctf60/g_phys.c -- diff-derived from src/ctf/g_phys.ts
// (lmctf60/g_phys.c vs quake-2/ctf/g_phys.c: 162-line diff of 956 total).
//
// Real deltas from src/ctf/g_phys.ts:
//  - SV_RunThink: the NULL-think guard now `return`s after gi.error instead
//    of falling through into `ent->think(ent)`.
//  - SV_FlyMove: the crease-check inner loop adds a
//    `!VectorCompare(planes[i], planes[j])` guard alongside `j != i`, so two
//    numerically distinct-index planes that happen to share the same normal
//    no longer reject the candidate velocity.
//  - SV_Push: a defensive NULL-pusher check was added (`gi.dprintf` + early
//    `return false`). No caller in this port ever passes a null pusher --
//    SV_Physics_Pusher's `part` loop variable is always non-null when it
//    calls SV_Push -- so this is unreachable here exactly as it is
//    effectively unreachable in the original engine (nothing in g_phys.c or
//    its callers constructs a NULL edict_t* to pass as pusher); ported
//    verbatim per the fidelity razor (observable behavior: never fires).
//  - SV_Physics_Toss: MOVETYPE_REFLECT (SKWiD MOD, lmctf60/g_local.h --
//    used by the plasma rifle's bouncing bolt, plasma.c, out of this
//    unit's SCOPE) is added to the movetypes exempted from gravity, gets
//    its own backoff of 2.0 ("get out of bsp"), and skips the whole
//    "stop if on ground" block entirely (that block is now wrapped in
//    `if (ent->movetype != MOVETYPE_REFLECT)`).
//  - G_RunEntity: MOVETYPE_REFLECT is added to the switch's
//    MOVETYPE_TOSS/BOUNCE/FLY/FLYMISSILE case group (all dispatch to
//    SV_Physics_Toss).
//  - SV_Physics_Step: the airborne-monster ground check
//    (`if (!ent->groundentity) M_CheckGround(ent);`) is now wrapped in
//    `#ifdef MONSTERS_OK`/`#endif` and dropped (MONSTERS_OK is not defined
//    for this mod), plus `sqrt` -> `sqrtf` (no-op here, both are
//    `Math.sqrt`) and a `if (!ent->inuse) return;` added after
//    `G_TouchTriggers(ent)`. Moot: see below, this function is a throwing
//    stub in this port regardless.
// Every other diff line is cosmetic (license header dropped, whitespace).
//
// SV_Physics_Step is the MOVETYPE_STEP handler -- monster-walking-with-
// ground-checks physics, used exclusively by monsters (players/items never
// use MOVETYPE_STEP). It calls M_CheckGround (g_monster.c) and
// M_CheckBottom (m_move.c); neither src/lmctf/g_monster.ts nor
// src/lmctf/m_move.ts exists in this port (no monster AI/movement
// subsystem in this LM_CTF family port, per this unit's brief -- the
// foundation's documented "MONSTERS_OK dead code stays dropped" ruling).
// No entity in this port is ever spawned with MOVETYPE_STEP, so
// G_RunEntity's MOVETYPE_STEP case is unreachable in practice; per rule 12
// (never create a file at a sibling module's real path) this throws a
// documented stub instead of fabricating monster ground-check logic.
// SV_AddRotationalFriction has no monster dependency of its own (it is a
// plain vector/friction helper) and is ported fully even though its only
// caller in the original source is SV_Physics_Step.
...
import { Com_sprintf, type CvarT, MASK_SOLID, MASK_WATER, YAW, CHAN_AUTO } from "../shared/q_shared";
import {
  FL_TEAMSLAVE,
  FRAMETIME,
  gameCvars,
  ...
  for (let bumpcount = 0; bumpcount < numbumps; bumpcount++) {
    ...
    for (i = 0; i < numplanes; i++) {
      ...
      for (j = 0; j < numplanes; j++)
        if (j !== i && VectorCompare(planes[i], planes[j]) === 0) {
...
export function SV_Push(pusher: EdictT, move: Vec3, amove: Vec3): boolean {
  // lmctf60 adds this defensive NULL-pusher guard (`__func__` in the
  // original dprintf format string is spelled out literally here, matching
  // SV_Physics_Pusher's existing gi.error citation for the same reason).
  // `pusher`'s TS type is the non-nullable `EdictT` -- SV_Physics_Pusher's
  // `part` loop variable is always non-null at every call site below -- so
  // this never actually fires in this port, matching the original: nothing
  // in g_phys.c or its callers ever constructs a NULL edict_t* to pass
  // here either.
  if (!pusher) {
    gi.dprintf("NULL pusher passed to SV_Push");
    return false;
  }
...
// SV_Physics_Step is the MOVETYPE_STEP handler -- see this file's header
// comment. It depends on M_CheckGround (src/lmctf/g_monster.ts, does not
// exist) and M_CheckBottom (src/lmctf/m_move.ts, does not exist): no
// monster AI/movement subsystem exists in this LM_CTF family port. No
// entity in this port is ever spawned with MOVETYPE_STEP -- players and
// items exclusively use MOVETYPE_TOSS/BOUNCE/FLY/FLYMISSILE/REFLECT,
// PUSH/STOP, NONE, or NOCLIP -- so G_RunEntity's MOVETYPE_STEP case is
// unreachable in this port in practice. This throws rather than
// fabricating monster ground-check/friction logic that has nowhere real
// to live yet.
export function SV_Physics_Step(_ent: EdictT): void {
  throw new Error(
    "SV_Physics_Step: requires M_CheckGround (src/lmctf/g_monster.ts) and M_CheckBottom (src/lmctf/m_move.ts), neither of which exists in this port -- no monster subsystem in this LM_CTF family port",
  );
}
