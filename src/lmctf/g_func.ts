// Ports lmctf60/g_func.c -- diff-derived from src/ctf/g_func.ts
// (lmctf60/g_func.c vs quake-2/ctf/g_func.c: 146-line diff of 2033 total).
//
// Real delta from src/ctf/g_func.ts: train_next's teleport path_corner
// branch now sets `self->s.event = EV_OTHER_TELEPORT;` right before
// gi.linkentity(self) in the teleport-and-continue case -- ctf's g_func.c
// never set this event, so the client never played/rendered the teleport
// effect for a train hitting a teleport path_corner; lmctf60 fixes that.
// Ported here as EntityEventT.EV_OTHER_TELEPORT, the same enum member
// already used the same way by src/ctf/g_misc.ts's teleporter code.
//
// Every other diff line is cosmetic and reproduced as a no-op: license
// header dropped, several `float` literals gained an `f` suffix (e.g.
// `1.0/FRAMETIME` -> `1.0f/FRAMETIME`, `0.1` -> `0.1f`), a stray `;` after
// a function body removed, an "allready" -> "already" comment typo fixed,
// and door_use_areaportals's `while ((t = G_Find(...)))` was restructured
// into an assign-then-loop shape purely to silence a level-4 compiler
// warning about assignment in a conditional -- ctf's TS port already uses
// the non-assigning `while ((t = G_Find(...)) !== null)` idiom here, so no
// change was needed on this side.
//
// One duplicate statement in the lmctf60 source is NOT reproduced:
// SP_func_door_secret computes `length = fabsf(DotProduct(forward,
// ent->size));` twice in a row. DotProduct/fabsf are pure (no side
// effects, no randomness) and both calls use identical inputs, so the
// second assignment is a strict no-op -- it cannot change observed
// behavior. Per PORTING.md's fidelity razor (observable behavior, not
// source text), this port keeps the single assignment already present in
// src/ctf/g_func.ts.
