...
import { CountPlayers, G_ProjectSource2, PickCoopTarget, PredictAim, blocked_checkshot, realrange, vectoangles2, vectoyaw2 } from "./g_newai";
import { G_FreeEdict, G_ProjectSource, G_Spawn } from "./g_utils";
import { CreateGroundMonster, FindSpawnPoint, SpawnGrow_Spawn } from "./g_monster";
...
// ThrowSmallStuff/ThrowWidowGibSized are defined in m_widow2.ts (this
// round's sibling file); Widowlegs_Spawn below needs them. m_widow2.ts also
// imports from this module (stalker_mins/stalker_maxs, WidowCalcSlots,
// widow_start_spawn, WidowPowerups), so this is a real, deliberate circular
// import between the two -- safe under ES module semantics because every
// cross-reference is only read inside a function body (never at module
// top-level evaluation time), exactly like rogue's own single-translation-
// unit m_widow.c/m_widow2.c cross-calls.
import { ThrowSmallStuff, ThrowWidowGibSized } from "./m_widow2";
...
// trace_t.ent recovery idiom (see g_monster.ts's/m_stalker.ts's own copy):
...
// Module-local per PORTING.md. (rogue/m_widow.ts's Widow_CheckAttack compares
// `tr.ent !== enemy` directly -- a lightweight Edict against a full EdictT,
// which can never be reference-equal even for the same entity; corrected
// here to the traceEdict(tr.ent) !== enemy idiom used everywhere else in
// this codebase, matching the equivalent check in m_carrier.ts's
// Carrier_CheckAttack and g_ai.ts's M_CheckAttack.)
...
// ****************************
//
// Ported from rogue/g_spawn.c (outside this round's edit ownership -- see
// m_stalker.ts and this porting round's report for the g_monster.ts-hosted
// CreateMonster/CreateGroundMonster/FindSpawnPoint/etc. family). This one
// prop-entity spawner is only ever called from spawn_out_do below, so it's
// hosted here directly instead, alongside the ThrowSmallStuff/
// ThrowWidowGibSized helpers (m_widow2.ts) it depends on.
...
function widowlegs_think(self: EdictT): void {
  ...
  if (level.time > self.wait) {
    ...
    ThrowWidowGibSized(self, "models/monsters/blackwidow/gib1/tris.md2", 80 + Math.floor(random() * 20.0), GIB_METALLIC, point, 0, true);
    ThrowWidowGibSized(self, "models/monsters/blackwidow/gib2/tris.md2", 80 + Math.floor(random() * 20.0), GIB_METALLIC, point, 0, true);
    ...
    ThrowWidowGibSized(self, "models/monsters/blackwidow/gib1/tris.md2", 80 + Math.floor(random() * 20.0), GIB_METALLIC, point, 0, true);
    ThrowWidowGibSized(self, "models/monsters/blackwidow/gib2/tris.md2", 80 + Math.floor(random() * 20.0), GIB_METALLIC, point, 0, true);
    ThrowWidowGibSized(self, "models/monsters/blackwidow/gib3/tris.md2", 80 + Math.floor(random() * 20.0), GIB_METALLIC, point, 0, true);
...
registerSaveFunction("m_widow:widowlegs_think", widowlegs_think);
