...
// kills builds toward a free Quad Damage. In the original game this is
// wired into DMGame (the dispatch table type/singleton declared in
// g_local.ts) by g_newdm.c's InitGameRules when gamerules is 2 (RDM_TAG);
// that wiring lives in g_newdm.ts, owned by another unit in this port
// effort -- see the port report for what still needs to happen there
// (assigning DMGame.* to the Tag_* functions below) and for the "Tag
// Token" itemlist entry and `gamerules` cvar this file needs that this
// unit does not own.
//
// Only the classname this unit was asked to port -- dm_tag_token -- needs
// this file; it drags in the whole ruleset because SP_dm_tag_token IS that
// ruleset's entry point (Tag_PostInitSetup spawns one if none exists on
// the map, matching the original wiring).
//
...
// -------------------------------------------------------------------------
registerSaveFunction("dm_tag:Tag_Respawn", Tag_Respawn);
registerSaveFunction("dm_tag:Tag_MakeTouchable", Tag_MakeTouchable);
registerSaveFunction("dm_tag:Tag_PickupToken", Tag_PickupToken);
