...
export function G_SetClientEffects(ent: EdictT): void {
  ...
  if (client !== null) {
    ...
    // RERELEASE CONTENT PORT -- item_invisibility (the cloak).
    //
    // The rerelease draws this with a dedicated fade shader driven by
    // `invisibility_fade_time`, which protocol 34 has no representation
    // for. RF_TRANSLUCENT is the closest thing the classic renderer and
    // the classic protocol both already understand, so the cloak reads as
    // translucency rather than the rerelease's ramped fade -- the player
    // is visibly cloaked, just not with the rerelease's exact look. The
    // same >30-or-blink tail every other powerup here uses gives the
