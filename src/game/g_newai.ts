...
// Ported from rogue/g_newai.c (GNU GPL v2 or later).
//
...
// -- rogue/g_newai.c has no `#ifdef`-gated dead sections.
//
...
//
// Deliberate deviations from src/rogue/g_newai.ts (this port's ownership
// boundary -- see the porting brief for this round):
// - `SP_hint_path` itself is NOT defined here. hint_path's classname-spawn
//   function is owned by a different concurrent porting worker (per the
//   coordinator's split of the 104-classname list); everything else this
//   file needs to support hint paths (hintpath_findstart/other_end/go/stop,
//   monsterlost_checkhint, hint_path_touch, InitHintPaths, and the module-
//   level hint_path_start/num_hint_paths/hint_paths_present state) is ported
//   here so whoever owns SP_hint_path can `import { hint_path_touch } from
//   "./g_newai"` and wire `self.touch = hint_path_touch` themselves, and can
//   call `InitHintPaths()` from wherever InitGame lives in this port.
// - `cleanupHealTarget` (used once, in TargetTesla) lives in rogue/g_combat.c
//   there, but src/game/g_combat.ts is outside this round's edit ownership.
//   It's implemented in src/game/g_monster.ts instead (this round's owner)
//   and imported from there. See this file's final report for the two
//   g_combat.c call sites (Killed()) that still need it wired up by whoever
//   owns g_combat.ts.
// - `blocked_checkshot`'s `monster_parasite`-specific branch (drains through
//   `parasite_drain_attack_ok` from m_parasite.c) is NOT ported: none of this
//   round's 8 monsters is monster_parasite, and src/game/m_parasite.ts
//   (outside this round's ownership) does not export
//   `parasite_drain_attack_ok`. If monster_parasite is ported later and
//   wants to reuse this file's blocked_checkshot, that branch needs to be
//   re-added by whoever owns m_parasite.ts, importing blocked_checkshot from
//   here.
// - `vectoyaw2`/`vectoangles2`/`G_ProjectSource2` (rogue/g_utils.c additions)
//   are hosted locally in this file instead of src/game/g_utils.ts, which is
//   outside this round's edit ownership. They are pure, self-contained
//   float-precision siblings of g_utils.ts's existing vectoyaw/vectoangles/
//   G_ProjectSource and behave identically to src/rogue/g_utils.ts's copies.
...
import { G_Find, G_FreeEdict, G_ProjectSource, G_Spawn, vtos } from "./g_utils";
import { M_ChangeYaw } from "./m_move";
import { cleanupHealTarget, monster_done_dodge } from "./g_monster";

// ---------------------------------------------------------------------
// rogue/g_utils.c additions hosted here -- see the file-top deviation note.
// ---------------------------------------------------------------------

// rogue's float-precision sibling of g_utils.ts's vectoyaw: skips the
// `(int)` truncation the base version applies.
export function vectoyaw2(vec: Vec3): number {
  ...
  if (/*vec[YAW] == 0 &&*/ vec[PITCH] === 0) {
    ...
    yaw = (Math.atan2(vec[YAW], vec[PITCH]) * 180) / Math.PI;
    if (yaw < 0) yaw += 360;
  }
...
// rogue's float-precision sibling of g_utils.ts's vectoangles, additionally
// fixed to correct for a pitch of 0 (the base version's `else yaw = -90;`
// case for a vec[2] > 0 straight-up vector).
export function vectoangles2(vec: Vec3, angles: Vec3): void {
  ...
  if (vec[1] === 0 && vec[0] === 0) {
    ...
    pitch = (Math.atan2(vec[2], forward) * 180) / Math.PI;
...
// ROGUE
export function G_ProjectSource2(point: Vec3, distance: Vec3, forward: Vec3, right: Vec3, up: Vec3, result: Vec3): void {
...
export function blocked_checkshot(self: EdictT, shotChance: number): boolean {
  ...
  // NOTE: rogue/g_newai.c has a monster_parasite-specific branch here
  // (parasite_drain_attack_ok) -- not ported; see this file's top-of-file
  // deviation note.
...
// hint_path_touch - someone's touched the hint_path. Exported (rather than
// wired up here) since SP_hint_path itself -- the classname spawn function
// that assigns `self.touch = hint_path_touch` -- is owned by a different
// concurrent porting worker; see this file's top-of-file deviation note.
...
// InitHintPaths - Called by InitGame to enable quick exits if valid. Exported
// for whoever wires up game init to call after the level's entities have
// spawned; see this file's top-of-file deviation note.
...
// -------------------------------------------------------------------------
// Savegame function/mmove registry -- see the tail of m_soldier.ts for the
// idiom. The touch/think callbacks exported from this module that can end up
// referenced by a saved edict are registered here.
// -------------------------------------------------------------------------

import { registerSaveFunction } from "./g_save";

registerSaveFunction("g_newai:hint_path_touch", hint_path_touch);
registerSaveFunction("g_newai:badarea_touch", badarea_touch);
